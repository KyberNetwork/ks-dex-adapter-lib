...
Below are a few assumptions you must follow before implementing your own adapter:
...
- If your integration does not transfer the output tokens directly to the recipient, just leave them inside the adapter contract; you don't need to explicitly transfer them.
- The adapter supports native tokens as input and output tokens, through the symbolic address `0xEeeeeEeeeEeEeeEeEeEeeEEEeeeeEeeeeeeeEEeE` (`TokenHelper.NATIVE_ADDRESS`).
